...
class Company extends CI_Controller
{
    ...
    function save($id = - 1)
    {
        ...
        $params = array(
            ...
            'file_stampel' => html_escape($file_stampel),
        ...
        if (isset($_POST) && count($_POST) > 0) {
            if (strlen($_FILES['file_stampel']['name']) > 0 && $_FILES['file_stampel']['size'] > 0) {
                if (! $this->upload->do_upload('file_stampel')) {
                    $error = array(
                        'error' => $this->upload->display_errors()
                    );
                } else {
                    $file_stampel = "uploads/images/company/" . $_FILES['file_stampel']['name'];
                    $params['file_stampel'] = $file_stampel;
                }
            } else {
                unset($params['file_stampel']);
            }
        }
