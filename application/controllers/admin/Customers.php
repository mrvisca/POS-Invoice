...
class Customers extends CI_Controller
{
    ...
    function save($id = - 1)
    {
        $params = array(
            ...
            'phone_no1' => html_escape($this->input->post('phone_no1')),
            'phone_no2' => html_escape($this->input->post('phone_no2')),
            'deskripsi' => html_escape($this->input->post('deskripsi')),
            'website' => html_escape($this->input->post('website')),
            'maps' => html_escape($this->input->post('maps'))
        );
        $data['id'] = $id;
        // update
        if (isset($id) && $id > 0) {
    ...
    function details($id)
    {
        // item_purchase
        $this->db->reset_query();
        $data['invoice'] = $this->db->get_where('invoice', array(
            'customers_id' => $id
        ))->result_array();
    ...
    function search($start = 0)
    {
        ...
        $this->db->or_like('nick_name', $key, 'both');
        ...
        $this->db->or_like('phone_no1', $key, 'both');
        $this->db->or_like('phone_no2', $key, 'both');
        $this->db->or_like('deskripsi', $key, 'both');
        $this->db->or_like('website', $key, 'both');
        $this->db->or_like('maps', $key, 'both');
    ...
    function export($export_type = 'CSV')
    {
        if ($export_type == 'CSV') {
            ...
            $stylesheet = file_get_contents(APPPATH . "vendor/mpdf/mpdf/data/lang2fonts.css");
